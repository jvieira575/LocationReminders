    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == SaveReminderFragment.ACTION_GEOFENCE_EVENT) {
            ...
            if (geofencingEvent.hasError()) {
                ...
                Timber.e("Error encountered in handing geofencing event: %s", errorMessage)
                return
            }
            ...
            if (geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
                ...
                when {
                    geofencingEvent.triggeringGeofences.isNotEmpty() -> {
                        ...
                        Timber.i("Calling intent service to send notification the user has entered the Geofence...")
                        GeofenceTransitionsJobIntentService.enqueueWork(context, intent)
                    }
                    else -> {

                        // Log that the triggering Geofence(s) were empty and exit the process
                        Timber.e("Triggering Geofences is empty. Exiting...")
                        return
                    }
                }
            }
